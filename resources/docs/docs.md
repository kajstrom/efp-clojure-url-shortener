...
The `home-routes` handler in the `efp-clojure-url-shortener.routes.home` namespace
...
The `home-page` function will in turn call the `efp-clojure-url-shortener.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `efp-clojure-url-shortener.handler` namespace:
...
Request middleware functions are located under the `efp-clojure-url-shortener.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `efp-clojure-url-shortener.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `efp-clojure-url-shortener.core` in some other namespace.
